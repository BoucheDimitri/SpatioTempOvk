...
kers = kernels.GaussianFuncKernel(kernelx=kernelx, mu=0.001, sigma=1000)
...
# Pickle regressor
with open(os.getcwd() + "/dumps/reg_funker.pkl", "wb") as o:
...
predtest = regressor.predict(dataintest.extract_subseq(0, 1), test_locs)
plt.figure()
plt.plot(test_locs, predtest.flatten(), label="predicted")
plt.plot(test_locs, dataouttest["y"][0], label="real")
